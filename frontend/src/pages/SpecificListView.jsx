  ...
  const FetchListMoviesData = useCallback(async () => {
    try{
      const options = {
        ...
        const response = await fetch(`${SERVER_URL}/lists/getList/${listID}/${userID}`, options);
  ...
  return (
    ...
    <BackButton />
      <div className='flex justify-between items-center pt-4 pb-8 text-custom-gold text-4xl font-bold'>
        ...
        <ListMoviesCard movies={moviesList} listID={listID} deleteIcon={false}/>
      )}
    </div>
  )
}
